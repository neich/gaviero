...
- [`GavieroMcpServer`](crates/gaviero-core/src/mcp/server.rs) — nine tools over [`McpEndpoint`](crates/gaviero-core/src/mcp/transport.rs): eight read-only (including `memory_ping`) plus write-adjacent `memory_flag` (trust demotion via the writer task; no `WriterHandle` on the server). Subprocess agents use `gaviero-mcp-shim`. DeepSeek does not.
