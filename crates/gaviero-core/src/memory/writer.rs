...
pub use super::writer_session::RollbackOutcome;
use super::writer_session::{
    abort_session_consolidate, apply_session_consolidate, process_consolidation_rollback,
    start_session_consolidate,
};
...
pub enum WriterMessage {
    ...
    /// Phase 1 of session consolidation failed after a `batch_id` was
    /// minted: the model call or the JSON parse did not succeed. Writes
    /// one `applied=0` audit row so `/consolidate history` can see it.
    SessionConsolidateAbort {
        run_id: String,
        batch_id: String,
        kind: String,
        error: String,
...
impl WriterMessage {
    ...
    pub fn kind(&self) -> &'static str {
        match self {
            ...
            WriterMessage::SessionConsolidateAbort { .. } => "SessionConsolidateAbort",
...
#[derive(Clone)]
pub(crate) struct Reenqueue(Weak<WriterHandleInner>);
...
impl Reenqueue {
    ...
    /// external caller's.
    pub(crate) fn send(&self, msg: WriterMessage) -> Result<()> {
        let inner = self
            .0
            .upgrade()
...
pub(crate) type InFlightRuns = Arc<std::sync::Mutex<std::collections::HashSet<String>>>;
...
impl WriterHandle {
    ...
    ///
    /// Flush does **not** wait for [`WriterMessage::SessionConsolidateApply`]
    /// or [`WriterMessage::SessionConsolidateAbort`] spawned after a
    /// `Queued` session-consolidate ack: those messages are enqueued later
    ...
    /// The ack means prep finished and the model call is running off the
    /// writer ([`WriteResult::Queued`]), not that operations have landed.
...
async fn writer_task(
    ...
    while let Some(msg) = rx.recv().await {
        ...
        // every prior *queued* message has been processed to completion.
        // It does not wait for SessionConsolidateApply/Abort that a
        // background task may still enqueue after a Queued ack.
...
async fn process_message(
    ...
    match msg {
        ...
        WriterMessage::SessionConsolidateAbort {
            run_id,
            batch_id,
            kind,
            error,
            ...
            let store = stores.workspace().clone();
            let res = abort_session_consolidate(&store, &run_id, &batch_id, &kind, &error).await;
            if let Ok(mut guard) = in_flight.lock() {
                guard.remove(&run_id);
            }
            res
