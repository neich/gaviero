...
impl GitPanelState {
    ...
    pub fn selected_path(&self) -> Option<&str> {
        match self.region {
            GitRegion::Unstaged => self.unstaged.get(self.unstaged_selected).map(|e| e.path.as_str()),
            GitRegion::Staged => self.staged.get(self.staged_selected).map(|e| e.path.as_str()),
    ...
    pub fn render(&self, area: Rect, buf: &mut RataBuf, focused: bool, _theme: &Theme) {
        ...
        for i in 0..unstaged_files as usize {
            ...
            if let Some(entry) = self.unstaged.get(i) {
                ...
                if mx < buf.area().right() && y < buf.area().bottom() {
                    buf[(mx, y)].set_char(marker).set_style(Style::default().fg(marker_color).bg(line_bg));
                }
        ...
        if let Some(ref err) = self.error_message {
            ...
            for (i, ch) in display.chars().enumerate() {
                let cx = area.x + i as u16;
                if cx < area.x + area.width && cx < buf.area().right() && err_y < buf.area().bottom() {
        ...
        if self.branch_picker_open {
            ...
            let max_rows = (area.height as usize).saturating_sub(4).min(filtered.len() + 2);
            ...
            if picker_y < buf.area().bottom() {
                let header = " Branches ";
                let h_style = Style::default().fg(picker_header_fg).bg(picker_bg).add_modifier(Modifier::BOLD);
